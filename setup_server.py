...
class basic_server_settings():
	...
	def __id_generator(self, size=6, chars=string.ascii_letters + string.digits + '!@#$%^&*()-_/.,<>?'):
	...
	def __set_password(self, password):
		...
		if password is None or password == '':
			...
			password = self.__id_generator(28)
	...
	def set_up_authorized_key(self, deploy_user, public_ssh_key):

		deploy_user = self.__set_deploy_user(deploy_user)

		authorized_keys = "/home/" + str(deploy_user) + '/.ssh/authorized_keys'

		if public_ssh_key is None or public_ssh_key == '':
...
Basic_server_settings.set_up_authorized_key(deploy_user, public_ssh_key)
